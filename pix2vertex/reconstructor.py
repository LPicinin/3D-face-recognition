...
class Reconstructor:
    def __init__(self, weights_path=None, detector=None):
        ...
        self.unet = pix2pix.UNet().to(device)
    ...
    def run_net(self, img):
        ...
        output = self.unet(input.to(device))
        output = output.add(1).div(2).mul(255)

        # Post Processing
        im_both = output.squeeze(0).cpu().detach().numpy().transpose().swapaxes(0, 1).copy()
        im_pncc = im_both[:, :, 0:3]
    ...
    def post_process(self, net_res):
        ...
        # Normalize a grade fixa de acordo com o resultado da rede, pois X, Y são realmente redundantes
        ...
        return {'Z': Z, 'X': X, 'Y': Y, 'Z_surface': Z_surface, 'mask': mask}
